   ...
   public class PaCong_parsing
    {
      ...
      public string UrlMath = "";
       ...
       public void parsing(int index,string Html) {
           ...
           List<string> HtmlParsingList = new List<string>();
           ...
           for (int i = 0; i < paCong_parsing_string.getHtml_parsing().Count; i++)
           {
              ...
              ItemHtml = paCong_parsing_string.getHtml_parsing()[i];
              //int I = ItemHtml.Split(new string[] { "src", "href" }, StringSplitOptions.None).Length;
               Html_parsing = Regex.Matches(Html, ItemHtml);
              if (ItemHtml.Split(new string[] { "src", "href" }, StringSplitOptions.None).Length > 1)
              {
                  //资源文件执行
                  foreach (Match item in Html_parsing)
                  {
                      
                      HtmlParsingList.Add(Img_Dowadlod(item, "", UrlMathlinshi));
                  ...
                  foreach (Match item in Html_parsing)
                  {
                      HtmlParsingList.Add(item.Groups[1].Value);
                      //ZiYuanDowadlod(item, "", UrlMathlinshi, i);
                  }
              }
           }
           paCong_parsing_string.setHtml_parsinglist(HtmlParsingList);
       }
       ...
       private string Img_Dowadlod(Match item, string UrlLinshi, string[] UrlMathlinshi)
       {
           if (Regex.Matches(item.Groups[1].Value, "http.*").Count == 0)
           {
               string[] linshi = item.Groups[1].Value.Split(new string[] { "../" }, StringSplitOptions.None);
